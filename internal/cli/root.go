...
func NewRoot() *cobra.Command {
	...
	root.PersistentFlags().DurationVar(&timeout, "timeout", browser.DefaultTimeout, "carrier request timeout")
...
func looksFedEx(number string) bool {
	if len(number) < 12 || len(number) > 22 {
		return false
	}
	for _, r := range number {
		if r < '0' || r > '9' {
